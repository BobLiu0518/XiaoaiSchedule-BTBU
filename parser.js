// Modified from:
// https://gitee.com/alovelygirl/aishedule/blob/master/%E5%BC%BA%E6%99%BA%E6%95%99%E5%8A%A1/parser.js
...
function resolveCourseConflicts(result) {
    ...
    result.forEach((singleCourse) => {
        singleCourse.weeks.forEach((week) => {
            singleCourse.sections.forEach((value) => {
                ...
                course.teacher =
                    singleCourse.teacher == undefined
                        ? ''
                        : singleCourse.teacher;
                course.position =
                    singleCourse.position == undefined
                        ? ''
                        : singleCourse.position;
            ...
            if (firstCourse.weeks[0] === allResult[i].weeks[0]) {
                if (firstCourse.sections[0] === allResult[i].sections[0]) {
                    ...
                    if (index === -1) {
                        ...
                        firstCourse.position +=
                            splitTag + allResult[i].position;
    ...
    while (contractResult.length != 0) {
        ...
        for (
            let i = 0;
            contractResult[i] !== undefined &&
            weekTag === contractResult[i].day;
            i++
        ) {
            if (
                ...
                if (
                    firstCourse.sections[firstCourse.sections.length - 1] +
                        1 ===
                ...
                .each(function (day, _) {
                    ...
                    kcco.forEach((con) => {
                            ...
                            if (font.eq(0).attr('title') == '教学楼') {
                                ...
                                if (
                                    font
                                        .eq(1)
                                        .text()
                                        .slice(jxlName.length)
                                        .search(jxlName) !== -1
                                ) {
                                    font.eq(1).text(
                                        font.eq(1).text().slice(jxlName.length)
                                    );
                                }
                            }
                        ...
                        switch (
                            ...
                            case '教师':
                                re.teacher = font.text();
                                re.teacher = re.teacher.replace(/讲师/, '');
                                re.teacher = re.teacher.replace(/副教授/, '');
                                re.teacher = re.teacher.replace(/教授/, '');
                                break;
                            case '教室':
                                re.position = font.text();
                                re.position = re.position.replace(/（.*）/, '');
                                break;
                            case '教学楼':
                                re.position = font.text();
                                break;
